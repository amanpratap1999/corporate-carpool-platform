...
export async function POST(
  ...
  if (!ride || ride.organization_id !== ctx.org.id) {
    return problemResponse(404, 'Ride Not Found', 'The requested ride was not found.', 'NOT_FOUND', `/api/v1/rides/${id}/complete`);
  }

  // Driver authorization check
  if (ride.driver_id !== ctx.user.id) {
    return problemResponse(403, 'Forbidden', 'Only the assigned driver can complete this ride.', 'FORBIDDEN', `/api/v1/rides/${id}/complete`);
  }
  ...
  try {
    ...
    return problemResponse(409, 'Cannot Complete Ride', errorMsg, 'INVALID_TRANSITION', `/api/v1/rides/${id}/complete`);
  }
}
