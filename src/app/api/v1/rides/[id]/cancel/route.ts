...
export async function POST(
  ...
  if (!ride || ride.organization_id !== ctx.org.id) {
    return problemResponse(404, 'Ride Not Found', 'The requested ride was not found.', 'NOT_FOUND', `/api/v1/rides/${id}/cancel`);
  }

  // Driver authorization check
  if (ride.driver_id !== ctx.user.id) {
    return problemResponse(403, 'Forbidden', 'Only the assigned driver can cancel this ride.', 'FORBIDDEN', `/api/v1/rides/${id}/cancel`);
  }
